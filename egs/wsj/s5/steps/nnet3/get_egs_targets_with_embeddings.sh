...
if [ $# != 4 ]; then
  echo "Usage: $0 [opts] <data> <embedding> <targets-scp> <egs-dir>"
  echo " e.g.: $0 data/train data/embedding data/train/mask.scp exp/tri4_nnet/egs"
  echo ""
...
for f in $data/feats.scp $embedding_dir/embedding.scp $targets_scp $extra_files; do
...
if [ ! -f $embedding_dir/utt2spk ]; then
  get_egs_program="nnet3-get-egs-dense-targets-with-embeddings --num-targets=$num_targets"
else
  get_egs_program="nnet3-get-egs-dense-targets-with-embeddings --num-targets=$num_targets --utt2spk=ark:$embedding_dir/utt2spk"
fi 
...
if [ $stage -le 3 ]; then
  ...
  $cmd $dir/log/create_valid_subset.log \
    ...
    $egs_opts "$valid_feats" scp:$embedding_dir/embedding.scp \
    "$valid_targets" \
    "ark:$dir/valid_all.egs" || touch $dir/.error &
  $cmd $dir/log/create_train_subset.log \
    $get_egs_program --frame-subsampling-factor=$frame_subsampling_factor \
    $egs_opts "$train_subset_feats" scp:$embedding_dir/embedding.scp \
...
if [ $stage -le 4 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/get_egs.JOB.log \
    $get_egs_program --frame-subsampling-factor=$frame_subsampling_factor \
    $egs_opts "$feats" scp:$embedding_dir/embedding.scp "$targets" \
    ark:- \| \
