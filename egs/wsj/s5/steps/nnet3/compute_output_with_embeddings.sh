...
if [ $# -ne 4 ]; then
  echo "Usage: $0 [options] <data-dir> <embedding-scp> <nnet-dir> <output-dir>"
...
for f in $data/feats.scp $model $embedding_scp; do
...
if [ $stage -le 2 ]; then
  $cmd $gpu_queue_opt JOB=1:$nj $dir/log/compute_output.JOB.log \
    nnet3-compute$suffix $gpu_opt $frame_subsampling_opt \
     --ivectors=scp:$embedding_scp \
