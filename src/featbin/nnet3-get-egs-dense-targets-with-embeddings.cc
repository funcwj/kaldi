...
static bool ProcessFile(const GeneralMatrix &feats,
                        const MatrixBase<BaseFloat> &embedding,
  ...
  for (size_t c = 0; c < chunks.size(); c++) {
    ...
    // add "auxin"
    // random sampled from embedding matrix
    int32 num_embeddings = embedding.NumRows();
    int32 random_index = RandInt(0, num_embeddings - 1);
    Matrix<BaseFloat> sampled_embedding(1, embedding.NumCols());
    sampled_embedding.Row(0).CopyFromVec(embedding.Row(random_index));
    eg.io.push_back(NnetIo("ivector", 0, sampled_embedding));
    ...
    eg.io.push_back(
        NnetIo("output", 0, targets_part, frame_subsampling_factor));
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "training.\n"
        "This program is similar to nnet3-get-egs-dense-targets, but prepare "
        "\"auxin\" vector in nnet3 egs which was randomly choose from "
        "embedding matrix. This was designed for something like "
        "speaker-aware training\n"
        "\n"
        "Usage:  nnet3-get-egs-dense-targets-with-embeddings --num-targets=<n> "
        "[options] <features-rspecifier> <embedding-rspecifier> "
        "<targets-rspecifier> <egs-out>\n"
        "\n";

    bool compress = true;
    int32 num_targets = -1, targets_length_tolerance = 2;
    ...
    std::string feature_rspecifier = po.GetArg(1),
                embedding_rspecifier = po.GetArg(2),
                target_rspecifier = po.GetArg(3),
    ...
    RandomAccessBaseFloatMatrixReaderMapped embedding_reader(
        embedding_rspecifier, utt2spk_rspecifier);
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      if (!target_reader.HasKey(key)) {
        ...
        const Matrix<BaseFloat> &target_matrix = target_reader.Value(key);
        // missing embeddings
        if (!embedding_reader.HasKey(key)) {
          KALDI_WARN << "No embeddings for key " << key;
          num_err++;
        } else {
          const Matrix<BaseFloat> &embedding = embedding_reader.Value(key);

          if (!ProcessFile(feats, embedding, target_matrix, key, compress,
